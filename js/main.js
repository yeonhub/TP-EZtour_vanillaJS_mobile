...
function tour(){
    const tour_btn = document.querySelectorAll('#tourNav .tour_btn strong')
    const con_box = document.querySelectorAll('.tourMain .con-box')

    tour_btn.forEach((ele,idx)=>{
        ele.addEventListener('click',e=>{
            // console.log('dd')
            tour_btn.forEach(item=>{
                item.classList.remove('on')
            })
            ele.classList.add('on')
            con_box.forEach(item=>{
                item.classList.remove('on')
            })
            con_box[idx].classList.add('on')
        })
    })
